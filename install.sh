...
mkdir -p "$INSTALL_DIR/hooks" "$INSTALL_DIR/bin" "$INSTALL_DIR/scripts" "$INSTALL_DIR/ui"
...
# 4. Create flag file (default: lazy) — don't overwrite existing preference
[ -f "$MODE_FILE" ] || echo "lazy" > "$MODE_FILE"

# 5. Patch claude-mem's hooks.json
python3 "$INSTALL_DIR/scripts/patch-hooks-json.py" --patch

# 6. Inject notify hook into ~/.claude/settings.json
python3 "$INSTALL_DIR/scripts/patch-hooks-json.py" --settings

# 7. Append memory mode protocol to ~/.claude/CLAUDE.md
...
# 9. Symlink CLI to somewhere on PATH (optional)
